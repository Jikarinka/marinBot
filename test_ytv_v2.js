...
async function testYtv() {
    ...
    try {
        ...
        async function executeConvert(url) {
            ...
            if (convertData.redirect === 1) return executeConvert(convertData.redirectURL);
            return convertData;
        }
        ...
        if (convertData.progressURL) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                console.log(`Attempt ${i+1}:`, finalData);
                if (finalData.progress >= 3) break;
                await new Promise(r => setTimeout(r, 3000));
            }
        }

        const downloadUrl = finalData.downloadURL || convertData.downloadURL;
        console.log("\nTesting Download URL:", downloadUrl);
        
        const dlRes = await fetch(downloadUrl, { headers: browserHeaders });
        const dlText = await dlRes.text();
        console.log("Download URL Response (first 200 chars):", dlText.substring(0, 200));

...
export const extractMessageData = (m, sock) => {
    ...
    // quotedSender: coba semua kemungkinan field
    let quotedSender = ''
    if (isQuoted) {
        quotedSender =
            contextInfo.participant ||           // pesan di grup (paling umum)
            contextInfo.remoteJid ||             // fallback
            contextInfo.mentionedJid?.[0] ||    // fallback lain
            ''
    }
    ...
    return {
        ...
        messageType,
        messageContent,
        text: messageContent,   // alias — dipakai passiveListener & ai-chat.js
        commandName,
        args,
        isQuoted,
        quotedMsg,
        ...
        // reply: kirim pesan dulu, BARU matikan typing
        // urutan: composing → sendMessage → paused
        reply: async (text) => {
            try { await sock.sendPresenceUpdate('composing', remoteJid) } catch (_) {}
            const result = await sock.sendMessage(remoteJid, { text }, { quoted: m });

        ...
        if (['public', 'register', 'gconly'].includes(feature)) {
            if (!user.isOwner) {
                return sock.sendMessage(msgData.remoteJid, {
                    text: `Maaf ya kak, cuma Owner Marin yang boleh atur fitur *${feature}*~ (｡T ω T｡)`
                }, { quoted: m });
            }
            ...
            if (feature === 'public') {
                ...
                return sock.sendMessage(msgData.remoteJid, {
                    text: `Selesai! Sekarang Marin ${status ? 'bisa diakses semua orang' : 'hanya merespon Owner (Mode Maintenance)'} yaa~ (˶˃ ᵕ ˂˶)`
                }, { quoted: m });
            } else if (feature === 'register') {
                await setting.update({ is_register: status });
                return sock.sendMessage(msgData.remoteJid, {
                    text: `Selesai! Fitur *registrasi* sekarang sudah Marin ${status ? 'aktifkan (wajib daftar)' : 'matikan (semua bebas akses)'} yaa~ (๑>ᴗ<๑)`
                }, { quoted: m });
            } else if (feature === 'gconly') {
                await setting.update({ is_gconly: status });
                return sock.sendMessage(msgData.remoteJid, {
                    text: `Selesai! Mode *Group Only* sekarang sudah Marin ${status ? 'aktifkan (hanya merespon di grup)' : 'matikan (merespon di grup & private)'} yaa~ (๑>ᴗ<๑)`
                }, { quoted: m });
            }
        }
        ...
        if (['welcome', 'limit'].includes(feature)) {
            ...
            if (feature === 'welcome') {
                ...
                await sock.sendMessage(msgData.remoteJid, {
                    text: `Horeee! Fitur *welcome* sekarang sudah Marin ${status ? 'aktifkan' : 'matikan'} buat grup ini yaa~ (˶˃ ᵕ ˂˶)`
                }, { quoted: m });

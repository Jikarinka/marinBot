# Marin-WABot V2 - Agent Context & Guidelines
...
- **Sentralisasi Pesan**: Gunakan variabel pesan dari `config.js` (seperti `config.MARIN_MSG_OWNER`, `config.MARIN_MSG_QUOTED`, dll) untuk respon standar. Hindari menulis ulang pesan (hardcoded) yang sudah didefinisikan di `.env`/`config.js`.
...
- **API Service**: Gunakan base URL `config.API_MARIN` untuk semua integrasi API Marin.
